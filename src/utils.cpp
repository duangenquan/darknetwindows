...
{
    size_t i;
    unsigned char *swp = (unsigned char*)calloc(1, size);
    for(i = 0; i < n-1; ++i){
    ...
    for(c = line, p = line; !done; ++c){
        ...
        if(*c == ',' || done){
            *c = '\0';
			...
			if(p == c) field[count] = _FPCLASS_QNAN;//nan("");
			if(end != c && (end != c-1 || *end != '\r')) field[count] = _FPCLASS_QNAN; //nan(""); //DOS file formats!
            p = c+1;
            ++count;
        }
    }
